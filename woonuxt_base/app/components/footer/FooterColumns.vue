      ...
      <div v-for="(link, index) in links" :key="index">
        ...
        <ul role="list" :class="ui.list">
          <li v-for="(subLink, subIndex) of link.children" :key="subIndex">
            <ULink v-bind="getULinkProps(subLink)" :class="ui.base" :active-class="ui.active" :inactive-class="ui.inactive" @click="subLink.click">
              {{ subLink.label }}

              <Icon v-if="subLink.target === '_blank'" :name="ui.externalIcon.name" :class="ui.externalIcon.base" />
            </ULink>
          </li>
        </ul>
      </div>
