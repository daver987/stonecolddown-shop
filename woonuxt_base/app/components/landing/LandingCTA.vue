...
const config = computed(() => {
  ...
  const container: string = twJoin('', props.align === 'center' && 'text-center', props.align === 'right' && 'lg:order-last');

  const base = twJoin('flex flex-col', props.align !== 'center' && 'lg:grid lg:grid-cols-2 lg:items-center', 'gap-16 sm:gap-y-24');

  const padding: string = props.card ? 'py-24 sm:py-32 sm:px-16' : 'py-24 sm:py-32 px-6 lg:px-8';

  const links: string = twJoin('mt-10 flex items-center gap-x-6', props.align === 'center' && 'justify-center');
  ...
  return {
    ...
    title: 'text-3xl font-heading tracking-tight text-amber-400 sm:text-4xl',
    description: 'mt-6 text-lg/8 text-gray-600 dark:text-gray-300 text-center',
    links,
  };
});

const { ui, attrs } = useUI('landing.cta', toRef(props, 'ui'), config, toRef(props, 'class') as Ref<string>, true);
</script>
