...
const productSlug = computed(
	() => `/product/${decodeURIComponent(props.item.product.node.slug)}`,
);
...
const imgSrc = computed(
	() =>
		...
		props.item.product.image?.sourceUrl ||
		FALLBACK_IMG,
);
const regularPrice = computed(() =>
	Number.parseFloat(productType.value.rawRegularPrice),
);
const salePrice = computed(() =>
	Number.parseFloat(productType.value.rawSalePrice),
);
