...
const props = defineProps({
	...
	ui: {
		type: Object as PropType<
			Partial<typeof config.value & typeof accordionConfig>
		>,
		default: () => ({}),
	},
});
...
const items = computed(() =>
	props.links?.map((link) => {
		const defaultOpen =
			!props.defaultOpen ||
			(typeof props.defaultOpen === "number" &&
				props.level < props.defaultOpen) ||
...
<template>
  <UAccordion v-bind="attrs" :key="route.path" :items="items as AccordionItem[]" :multiple="multiple" :ui="ui">
    ...
    <template v-for="({ label }, index) in links" :key="index" #[label.toLowerCase()]="{ item }">
      <NavigationTree :links="item.children" :level="level + 1" :default-open="defaultOpen" :multiple="multiple" :class="ui.tree" />
    </template>
  </UAccordion>
</template>
