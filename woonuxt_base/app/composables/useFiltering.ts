...
export function useFiltering() {
  ...
  function filterProducts(products: Product[]): Product[] {
    return products.filter((product) => {
      ...
      const productPrice = product.rawPrice ? Number.parseFloat([...product.rawPrice.split(',')].reduce((a, b) => String(Math.max(Number(a), Number(b))))) : 0
      ...
      const globalProductAttributes = runtimeConfig?.public?.GLOBAL_PRODUCT_ATTRIBUTES?.map((attribute: { slug: string }) => attribute.slug) || []
      const attributeCondition = globalProductAttributes
        .map((attribute: string) => {
          ...
          return !!product.terms?.nodes?.find((node) => node.taxonomyName?.toString() === attribute && attributeValues.includes(node.slug?.toString() || ''))
        })
