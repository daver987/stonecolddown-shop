...
export const useCountry = () => {
	...
	const allowedCountries = useState<CountriesEnum[] | null>(
		"allowedCountries",
		() => null,
	);
	const isLoadingAllowedCountries = useState<boolean>(
		"isLoadingAllowedCountries",
		() => false,
	);

	// State to store the countries to be shown - init with static countries
	const countriesToShow = useState<GeoLocation[]>(
		"countriesToShow",
		() => countries,
	);

	// State to store states for each country state
	const countryStatesDict = useState<{ [code: string]: GeoLocation[] }>(
		"countryStatesDict",
		() => ({}),
	);
	const isLoadingCountryStates = useState<{ [code: string]: boolean }>(
	...
	async function getStatesForCountry(countryCode: CountriesEnum) {
		if (
			...
			isLoadingCountryStates.value[countryCode]
		) {
			return;
		}
		...
		try {
			...
			console.error(
				`Failed to retrieve states for country ${countryCode}`,
				error,
			);
		} finally {
