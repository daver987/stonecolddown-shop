...
onMounted(async () => {
	try {
		...
		const errorMessage = (error as { gqlErrors?: { message: string }[] })
			?.gqlErrors?.[0]?.message;
...
useSeoMeta({
	...
	ogDescription: () =>
		product.value?.shortDescription ||
		`Get your hands on ${product.value?.name}. Unique merchandise featuring Natasha Smith's distinctive tattoo designs.`,
	...
	twitterDescription: () =>
		`Discover ${product.value?.name} from Stone Cold Down. Tattoo-inspired product by fine line artist Natasha Smith.`,
